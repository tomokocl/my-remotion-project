...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    input[type="url"]:focus { border-color: #41c9b0; }
    button {
      width: 100%; padding: 14px; background: #41c9b0; color: white;
    ...
    a.dl-btn {
      display: block; margin-top: 12px; padding: 12px; background: #111;
...
<body>
  <div class="card">
    ...
    <form id="form">
      <label>note の URL</label>
      <input type="url" id="url" placeholder="https://note.com/..." required>
      <label>Cookie ファイル（JSON）</label>
      <input type="file" id="cookies" accept=".json">
  ...
  <script>
    document.getElementById('form').addEventListener('submit', async (e) => {
      ...
      const f = document.getElementById('cookies').files[0];
      ...
      try {
        ...
        status.innerHTML = '変換完了！<a class="dl-btn" href="' + url + '" download="note-article.pdf">PDFをダウンロード</a>';
      } catch (err) {
...
server.post('/convert', upload.single('cookies'), async (req, res) => {
  const { url } = req.body;
  ...
  if (!chromePath) {
    ...
    return res.status(500).json({ error: 'Chromeが見つかりません。Google Chromeをインストールしてください。' });
  }

  // playwright-core を動的 import（ESM モジュールのため）
  ...
  try {
    ...
    await page.addStyleTag({
      content: `@media print {
        ...
        [class*="modal"],[class*="toast"],[class*="banner"],[class*="ad"] { display:none!important; }
    ...
    res.set({ 'Content-Type': 'application/pdf', 'Content-Disposition': 'attachment; filename="note-article.pdf"' });
